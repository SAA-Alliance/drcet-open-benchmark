#!/usr/bin/env python3
"""Check DRCET-4 operator-splitting convergence evidence pack v1."""
...
DEFAULT_DIR = REPO / "docs/benchmarks/drcet4_operator_splitting_convergence_v1_20260816"
ARTIFACT_ID = "DRCET4_OPERATOR_SPLITTING_CONVERGENCE_V1"
STATUS_FILE = "DRCET4_OPERATOR_SPLITTING_CONVERGENCE_STATUS.json"
...
EXPECTED_STATUS_WITH_CHERNOFF = "PROTOCOL_FULL_LINEAR_EXACT_CONTAINER_AND_REMIZOV_CHERNOFF_LANE_VALIDATED_PRODUCTION_ARIN22_PENDING"
STEP_COUNTS = [4, 8, 16, 32, 64, 128, 256, 512]
CHERNOFF_STEP_COUNTS = [8, 16, 32, 64, 128, 256, 512, 1024]
SOLVERS = {
    "LIE_AB",
    "LIE_BA",
    "STRANG_ABA",
...
REQUIRED_FILES = {
    ...
    "remizov_chernoff_fixture_contract.csv",
    "remizov_chernoff_convergence_results.csv",
    "remizov_chernoff_convergence_summary.csv",
    "remizov_chernoff_tangent_stability_summary.csv",
    "remizov_chernoff_protected_container_input.json",
    "remizov_chernoff_protected_container_output.json",
    "remizov_chernoff_protected_container_probe.json",
    "production_arin22_remizov_chernoff_container_input.json",
    "production_arin22_remizov_chernoff_container_output.json",
    "production_arin22_remizov_chernoff_container_probe.json",
    ...
    "remizov_chernoff_loglog_convergence.svg",
    "README.md",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check DRCET-4 operator-splitting convergence v1 pack")
    ...
    chernoff_fixture_rows = read_csv(pack_dir / "remizov_chernoff_fixture_contract.csv")
    chernoff_result_rows = read_csv(pack_dir / "remizov_chernoff_convergence_results.csv")
    chernoff_summary_rows = read_csv(pack_dir / "remizov_chernoff_convergence_summary.csv")
    chernoff_tangent_rows = read_csv(pack_dir / "remizov_chernoff_tangent_stability_summary.csv")
    manifest_sha, _manifest = check_manifest(pack_dir)

    if status.get("artifact_id") != ARTIFACT_ID:
        fail("status artifact_id mismatch")
    if status.get("status") not in {EXPECTED_STATUS, EXPECTED_STATUS_WITH_CONTAINER, EXPECTED_STATUS_FULL_CONTAINER_A6, EXPECTED_STATUS_WITH_CHERNOFF}:
    ...
    if len(chernoff_fixture_rows) != 6:
        fail(f"remizov_chernoff_fixture_contract rows must be 6, got {len(chernoff_fixture_rows)}")
    if len(chernoff_result_rows) != 6 * len(CHERNOFF_STEP_COUNTS):
        fail(f"remizov_chernoff_convergence_results rows must be {6 * len(CHERNOFF_STEP_COUNTS)}, got {len(chernoff_result_rows)}")
    if len(chernoff_tangent_rows) != 6:
        fail(f"remizov_chernoff_tangent_stability_summary rows must be 6, got {len(chernoff_tangent_rows)}")
    if len(chernoff_summary_rows) != 1:
        fail("remizov_chernoff_convergence_summary must contain exactly one row")
    if {int(r["step_count_n"]) for r in chernoff_result_rows} != set(CHERNOFF_STEP_COUNTS):
        fail("Remizov-Chernoff step counts mismatch")
    if {r["chernoff_function_G"] for r in chernoff_fixture_rows} != {"I + tL"}:
        fail("Remizov-Chernoff fixture contract must bind G(t)=I+tL")
    if {r["semigroup_reference"] for r in chernoff_fixture_rows} != {"expm(tL)"}:
        fail("Remizov-Chernoff fixture contract must bind S(t)=exp(tL)")
    if any(r["operator_contract_L"] != "bounded_matrix_generator" for r in chernoff_fixture_rows):
        fail("Remizov-Chernoff operator contract must be bounded_matrix_generator for this lane")
    for row in chernoff_result_rows:
        if row["lane_id"] != "DRCET4B_REMIZOV_CHERNOFF_CONVERGENCE_LANE":
            fail("Remizov-Chernoff lane_id mismatch")
        if row["status"] != "PASS":
            fail(f"Remizov-Chernoff result row did not PASS: {row['case_id']} n={row['step_count_n']} status={row['status']}")
        if not as_bool(row["tangent_check_pass"]):
            fail(f"Remizov-Chernoff tangent check failed for {row['case_id']} n={row['step_count_n']}")
        if not as_bool(row["stability_check_pass"]):
            fail(f"Remizov-Chernoff stability check failed for {row['case_id']} n={row['step_count_n']}")
        if not as_bool(row["certificate_E_n_le_B_n"]):
            fail(f"Remizov-Chernoff E_n <= B_n failed for {row['case_id']} n={row['step_count_n']}")
        if as_float(row["S0_identity_error"]) is None or as_float(row["S0_identity_error"]) > 1e-14:
            fail(f"Remizov-Chernoff S(0)=I check failed for {row['case_id']}")
        if as_float(row["bound_utilization_E_over_B"]) is None or as_float(row["bound_utilization_E_over_B"]) > 1.0:
            fail(f"Remizov-Chernoff bound utilization above 1 for {row['case_id']} n={row['step_count_n']}")
    for row in chernoff_tangent_rows:
        if row["status"] != "PASS":
            fail(f"Remizov-Chernoff tangent/stability row did not PASS: {row['case_id']}")
        observed = as_float(row["observed_convergence_rate"])
        if observed is None or not (0.75 <= observed <= 1.25):
            fail(f"Remizov-Chernoff observed convergence rate outside first-order band for {row['case_id']}: {observed}")
        if int(row["certificate_failures"]) != 0:
            fail(f"Remizov-Chernoff certificate failures present for {row['case_id']}")

    chernoff_summary = status.get("drcet4b_remizov_chernoff_convergence_lane") or {}
    if chernoff_summary.get("lane_id") != "DRCET4B_REMIZOV_CHERNOFF_CONVERGENCE_LANE":
        fail("status missing DRCET4B lane summary")
    if chernoff_summary.get("fixture_count") != 6 or chernoff_summary.get("result_rows") != 6 * len(CHERNOFF_STEP_COUNTS):
        fail("DRCET4B lane summary fixture/result counts mismatch")
    if chernoff_summary.get("step_counts") != CHERNOFF_STEP_COUNTS:
        fail("DRCET4B lane summary step counts mismatch")
    csv_lane_status = chernoff_summary_rows[0].get("status")
    if csv_lane_status != chernoff_summary.get("status"):
        fail("DRCET4B CSV summary status must match JSON status")
    chernoff_probe = read_json(pack_dir / "remizov_chernoff_protected_container_probe.json")
    chernoff_probe_input = read_json(pack_dir / "remizov_chernoff_protected_container_input.json")
    chernoff_probe_output = read_json(pack_dir / "remizov_chernoff_protected_container_output.json")
    if chernoff_probe_input.get("schema_id") != "saa.benchmark.drcet4.remizov_chernoff_container_input.v1":
        fail("Remizov-Chernoff container input schema mismatch")
    if chernoff_probe_output.get("schema_id") != "saa.benchmark.drcet4.remizov_chernoff_container_output.v1":
        fail("Remizov-Chernoff container output schema mismatch")
    if chernoff_probe.get("input_sha256") != sha256_file(pack_dir / "remizov_chernoff_protected_container_input.json"):
        fail("Remizov-Chernoff container input sha mismatch")
    if chernoff_probe.get("output_sha256") != sha256_file(pack_dir / "remizov_chernoff_protected_container_output.json"):
        fail("Remizov-Chernoff container output sha mismatch")
    chernoff_probe_status = chernoff_probe.get("status")
    if chernoff_summary.get("status") == "VALIDATED_PASS_REVIEW_ONLY_REMIZOV_CHERNOFF_CONVERGENCE_LANE":
        if chernoff_probe_status != "PASS_REVIEW_ONLY_NO_EGRESS_CONTAINER_REMIZOV_CHERNOFF_LANE":
            fail("DRCET4B VALIDATED/PASS requires protected no-egress container PASS")
        if chernoff_probe.get("container_runtime") != "docker" or chernoff_probe.get("network_mode") != "none":
            fail("DRCET4B container PASS must be docker/no-egress")
        if chernoff_probe.get("case_count") != 6 * len(CHERNOFF_STEP_COUNTS):
            fail("DRCET4B container case count mismatch")
        if chernoff_probe.get("covered_fixture_count") != 6 or chernoff_probe.get("total_fixture_count") != 6:
            fail("DRCET4B container fixture coverage mismatch")
        ratio = chernoff_probe.get("max_delta_to_tolerance_ratio")
        if not isinstance(ratio, (int, float)) or float(ratio) > 1.0:
            fail("DRCET4B container delta ratio missing or above tolerance")
        if status.get("status") != EXPECTED_STATUS_WITH_CHERNOFF:
            fail("overall DRCET-4 status must include Remizov-Chernoff validation when DRCET4B passes")
    else:
        if chernoff_summary.get("status") != "CONTRACT_DECLARED_WATCH":
            fail(f"unexpected DRCET4B non-pass status: {chernoff_summary.get('status')}")
        if chernoff_probe_status == "PASS_REVIEW_ONLY_NO_EGRESS_CONTAINER_REMIZOV_CHERNOFF_LANE":
            fail("DRCET4B container PASS cannot leave lane in CONTRACT_DECLARED_WATCH")
        if "drcet4b_remizov_chernoff_convergence_lane_not_validated" not in blockers:
            fail("DRCET4B watch status must keep not_validated blocker")

    production_probe = read_json(pack_dir / "production_arin22_remizov_chernoff_container_probe.json")
    production_input = read_json(pack_dir / "production_arin22_remizov_chernoff_container_input.json")
    production_output = read_json(pack_dir / "production_arin22_remizov_chernoff_container_output.json")
    production_summary = status.get("production_arin22_remizov_chernoff_container_probe") or {}
    nested_production_summary = chernoff_summary.get("production_arin22_protected_container_probe") or {}
    ...
    if production_output.get("input_sha256") != sha256_file(pack_dir / "production_arin22_remizov_chernoff_container_input.json"):
    ...
    print(json.dumps({
        ...
        "drcet4b_remizov_chernoff_convergence_lane": status.get("drcet4b_remizov_chernoff_convergence_lane"),
