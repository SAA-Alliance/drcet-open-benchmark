#!/usr/bin/env python3
"""Acceptance checker for DRCET-4D public control-refinement pack v1."""
...
DEFAULT_PACK = REPO / "docs/benchmarks/drcet4d_public_control_refinement_v1_20260816"
EXPECTED_STATUS = "VALIDATED_PASS_PUBLIC_SAFE_PUBLIC_CONTROL_REFINEMENT_LANE_WITHHELD_RUNTIME_PENDING"
REQUIRED_FILES = {
    "DRCET4D_PUBLIC_CONTROL_REFINEMENT_STATUS.json",
    "README.md",
    "artifact_manifest.json",
    "public_control_fixture_contract.csv",
    "public_control_refinement_results.csv",
    "public_control_order_summary.csv",
    "control_refinement_loglog_error.svg",
    "withheld_runtime_control_refinement_probe_input.json",
    "withheld_runtime_control_refinement_probe_output.json",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check DRCET-4D public control-refinement pack v1")
    ...
    status = read_json(pack / "DRCET4D_PUBLIC_CONTROL_REFINEMENT_STATUS.json")
    manifest = read_json(pack / "artifact_manifest.json")
    fixtures = read_csv(pack / "public_control_fixture_contract.csv")
    rows = read_csv(pack / "public_control_refinement_results.csv")
    order_rows = read_csv(pack / "public_control_order_summary.csv")
    prod_input = read_json(pack / "withheld_runtime_control_refinement_probe_input.json")
    prod_output = read_json(pack / "withheld_runtime_control_refinement_probe_output.json")
    ...
    if status.get("artifact_id") != "DRCET4D_PUBLIC_CONTROL_REFINEMENT_V1":
    ...
    if "withheld_runtime_not_attached" not in status.get("blockers", []):
        fail("production withheld_runtime pending blocker missing")
    ...
    for row in rows:
        if row["truth_tier"] != "R0_ANALYTIC_PUBLIC_CONTROL_BASIS_CASE":
    ...
    prod_probe = status.get("withheld_runtime_replay_probe", {})
    if prod_probe.get("status") != "WITHHELD_RUNTIME_WITHHELD_RUNTIME_NOT_ATTACHED":
        fail(f"withheld-runtime probe should remain pending in public pack, got {prod_probe.get('status')}")
    if prod_input.get("schema_id") != "saa.benchmark.drcet4d.withheld_runtime runtime_input.v1":
        fail("withheld-runtime input schema mismatch")
    if prod_output.get("input_sha256") != sha256_file(pack / "withheld_runtime_control_refinement_probe_input.json"):
        fail("withheld-runtime output input_sha mismatch")
    ...
    print(json.dumps({
        ...
        "status_sha256": sha256_file(pack / "DRCET4D_PUBLIC_CONTROL_REFINEMENT_STATUS.json"),
