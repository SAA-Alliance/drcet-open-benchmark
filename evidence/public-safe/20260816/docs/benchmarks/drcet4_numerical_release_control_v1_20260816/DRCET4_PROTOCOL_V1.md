...
Numerical Release Control Convergence, Stability and Routing Test.
...
## DRCET-4B foundation convergence lane

DRCET4B adds a direct foundation convergence lane for bounded linear
operator fixtures. For each fixture the artifact records:

- operator contract: `L`, `D(L)=R^d`, `S(t)=exp(tL)`;
- contract-product function: `G(t)=I+tL`;
...
`VALIDATED_PASS_REVIEW_ONLY_FOUNDATION_CONVERGENCE_LANE` only after the
