#!/usr/bin/env python3
"""Acceptance checker for DRCET-4C Chernoff-Remizov resolvent pack v1."""
...
REQUIRED_FILES = {
    ...
    "resolvent_half_plane_policy.json",
    "remizov_resolvent_fixture_contract.csv",
    "remizov_resolvent_lambda_sweep_results.csv",
    "remizov_resolvent_tangency_product_summary.csv",
    "remizov_resolvent_convergence_summary.csv",
    "remizov_resolvent_protected_container_input.json",
    "remizov_resolvent_protected_container_output.json",
    "production_arin22_resolvent_container_input.json",
    "production_arin22_resolvent_container_output.json",
    "production_arin22_resolvent_container_probe.json",
    "external_blind_replay_request.json",
    "external_blind_replay_receipt.json",
    "remizov_resolvent_loglog_convergence.svg",
    "remizov_resolvent_lambda_gate_sweep.svg",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check DRCET-4C Chernoff-Remizov resolvent pack v1")
    ...
    status = read_json(pack / "DRCET4C_CHERNOFF_REMIZOV_RESOLVENT_STATUS.json")
    manifest = read_json(pack / "artifact_manifest.json")
    fixture_rows = read_csv(pack / "remizov_resolvent_fixture_contract.csv")
    lambda_rows = read_csv(pack / "remizov_resolvent_lambda_sweep_results.csv")
    tangent_rows = read_csv(pack / "remizov_resolvent_tangency_product_summary.csv")
    convergence_rows = read_csv(pack / "remizov_resolvent_convergence_summary.csv")
    container_input = read_json(pack / "remizov_resolvent_protected_container_input.json")
    container_output = read_json(pack / "remizov_resolvent_protected_container_output.json")
    production_input = read_json(pack / "production_arin22_resolvent_container_input.json")
    production_output = read_json(pack / "production_arin22_resolvent_container_output.json")
    production_probe = read_json(pack / "production_arin22_resolvent_container_probe.json")
    ...
    if status.get("artifact_id") != "DRCET4C_CHERNOFF_REMIZOV_RESOLVENT_V1":
        fail("artifact_id mismatch")
    if status.get("status") != EXPECTED_STATUS:
        fail(f"unexpected status: {status.get('status')}")
    if status.get("contract_function_policy", {}).get("not_the_object") != "I+tL Euler-Chernoff proxy from DRCET-4B":
    ...
    if "production_ARIN22_kernel_validated_through_Remizov_route" not in status.get("must_not_claim", []):
    ...
    if status.get("silent_release_outside_certified_resolvent_half_plane_count") != 0:
        fail("silent release outside certified resolvent half-plane detected")
    fail_closed_count = int(status.get("fail_closed_outside_certified_resolvent_half_plane_rows", -1))
    if fail_closed_count <= 0:
        fail("certified resolvent half-plane must have fail-closed rows")
    if status.get("exact_resolvent_diagnostic_available_below_boundary_rows") != fail_closed_count:
        fail("below-boundary diagnostic exact resolvent readback must match fail-closed rows in this bounded fixture")
    ...
    if int(lambda_status_counts.get(SAFE_FAIL_CLOSED_OUTSIDE_RESOLVENT_HALF_PLANE, 0)) <= 0:
    ...
    if int(lambda_status_counts.get("PASS_RESOLVENT_CONVERGENCE", 0)) <= 0:
        fail("lambda status counts must include converged rows")
    convergence_status_counts = status.get("resolvent_convergence_status_counts", {})
    if not convergence_status_counts or sum(int(v) for v in convergence_status_counts.values()) != int(status.get("resolvent_convergence_rows", -1)):
        fail("resolvent convergence status counts do not sum to convergence rows")
    if int(convergence_status_counts.get("WATCH_RESOLVENT_ORDER", 0)) > 0 and status.get("watch_items") != ["near_boundary_quadrature_dominated_resolvent_order_row"]:
        fail("quadrature watch item must be explicit when convergence watch rows exist")
    if fnum(status.get("observed_resolvent_order_median"), "observed_resolvent_order_median") < 1.8:
    ...
    if any(r["chernoff_function_G"] == "I+tL" for r in fixture_rows):
    ...
    for row in lambda_rows:
        ...
        if inside:
            ...
            if row["status"] != SAFE_FAIL_CLOSED_OUTSIDE_RESOLVENT_HALF_PLANE:
            ...
            if row.get("stability_probe_pass") != "NOT_APPLICABLE_OUTSIDE_CERTIFIED_RESOLVENT_HALF_PLANE":
    ...
    bootstrap = status.get("resolvent_order_bootstrap_ci95", {})
    ...
    if probe.get("status") != "PASS_REVIEW_ONLY_NO_EGRESS_CONTAINER_REMIZOV_RESOLVENT_LANE":
    ...
    if container_input.get("schema_id") != "saa.benchmark.drcet4c.remizov_resolvent_container_input.v1":
        fail("container input schema mismatch")
    if container_output.get("schema_id") != "saa.benchmark.drcet4c.remizov_resolvent_container_output.v1":
        fail("container output schema mismatch")
    if container_output.get("input_sha256") != sha256_file(pack / "remizov_resolvent_protected_container_input.json"):
    ...
    if production_output.get("input_sha256") != sha256_file(pack / "production_arin22_resolvent_container_input.json"):
